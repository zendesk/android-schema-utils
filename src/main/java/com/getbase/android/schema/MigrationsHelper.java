...
public class MigrationsHelper {
  ...
  public void performMigrations(SQLiteDatabase db, TableMigration... migrations) {
    for (TableMigration migration : migrations) {
      ...
      String insertColumnsString = Joiner.on(",").join(Iterables.unmodifiableIterable(Iterables.concat(unmappedColumns, migration.mappings.keySet())));
      String selectColumnsString = Joiner.on(",").join(Iterables.unmodifiableIterable(Iterables.concat(unmappedColumns, migration.mappings.values())));
